...
// 👇 updated type to match API keys exactly
type SustainabilityResult = {
  row_number: number;
  UserId: string;
  product?: string;
  ...
  packaging_impact: string;
  origin?: string;
  sustainability_score: string;       // <-- lowercase s
  ingredient_impact: string;
  certifications: string;
  major_concerns: string;             // JSON string
  actionable_advice: string;          // JSON string
  suggested_alternative: string;      // JSON string
  summary: string;
};
      ...
      <DrawerContent className="h-full w-[90vw] min-w-4xl overflow-y-auto">
        <div className="flex flex-col h-full">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => router.refresh()} className="m-6" variant="outline">
              <RefreshCw />
            </Button>
          </div>
          <ScrollArea className="flex-1 p-6">
            {data && (
              <div className="space-y-6">
                {data.map((result, i) => {
                  const majorConcerns = parseJsonString(result.major_concerns);
                  const actionableAdvice = parseJsonString(result.actionable_advice);
                  const suggestedAlternative = parseSuggestedAlternative(result.suggested_alternative);
                    ...
                    <Card key={i} className="border border-gray-200">
                      <CardHeader className="pb-4">
                        <CardTitle className="text-lg font-semibold text-gray-900 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <CheckCircle className="h-5 w-5 text-green-600" />
                            {result.product ?? "Unknown"} - {result.brand ?? "Unknown"}
                          </div>
                          <Badge
                            className={`${getSustainabilityColor(result.sustainability_score)} border`}
                          >
                            {result.sustainability_score}
                          </Badge>
                        </CardTitle>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {majorConcerns.length > 0 && (
                          <div>
                            ...
                            <ul className="space-y-2">
                              {majorConcerns.map((concern: string, idx: number) => (
                          ...
                          <div className="bg-green-50 rounded-lg p-4">
                            ...
                            <div className="space-y-2">
                              <p className="text-sm">
                                ...
                                <span className="text-green-700">{suggestedAlternative.product}</span>
                              </p>
                              <p className="text-sm">
                                <span className="font-medium text-green-800">Brand:</span>{" "}
                                <span className="text-green-700">{suggestedAlternative.brand}</span>
                              </p>
                              <p className="text-sm">
                                <span className="font-medium text-green-800">Reason:</span>{" "}
                                <span className="text-green-700">{suggestedAlternative.reason}</span>
                              </p>
                            </div>
                          </div>
                        )}
                        ...
                        {actionableAdvice.length > 0 && (
                          <div>
                            ...
                            <ul className="space-y-2">
                              {actionableAdvice.map((advice: string, idx: number) => (
                                <li key={idx} className="flex items-start gap-2 text-sm text-gray-700">
                                  <span className="w-1.5 h-1.5 bg-blue-400 rounded-full mt-2 flex-shrink-0"></span>
                                  {advice}
                                </li>
                        ...
                        {result.certifications && result.certifications !== "None found" && (
                          <div>
                            <h4 className="font-medium text-gray-900 mb-2">Certifications</h4>
                            <p className="text-sm text-gray-700">{result.certifications}</p>
                          </div>
                        )}
                      </CardContent>
                    </Card>
